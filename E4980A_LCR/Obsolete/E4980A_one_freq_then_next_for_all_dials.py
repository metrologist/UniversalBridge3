...
# Feb 25, three frequency point added
# need more than 30 minutes to run

"""Not in use, as this program requests repeat dialing to different points,the best practice shall be
a few frequencies applied at one dial position"""
...
with open(output_file, mode='w', newline='') as file:
    ...
    # Writing header
    writer.writerow(["DUT", "frequency", "Measurement Condition", "Dial", "L", "L_Std", "R", "R_Std"])
...
# freq_list = [10, 1591.5494309189535, 20000]  # optional more Frequencies (in Hz)
freq_list = [1591.5494309189535, 1671.126902464901]  # three Frequency point (in 10k Omega and ± 5% )
dials = ["Dial 0", "Dial 1", "Dial 2", "Dial 3", "Dial 4", "Dial 5", "Dial 6", "Dial 7", "Dial 8", "Dial 9", "Dial 10"]
# dials = ["one dial only"]
for f in freq_list:

    # Loop for each dial at the certain frequency
    for dial in dials:
        next_dail_ready = input('dial to the right position, get ready for the next?')
        ...
        with open(output_file, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([DUT_description, f, condition, dial, avg_L, dev_L, avg_R, dev_R])
